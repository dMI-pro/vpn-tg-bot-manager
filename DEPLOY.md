...
## 11. Деплой через Docker (Рекомендуется)
Если на вашем сервере уже установлен Docker и Docker Compose, вы можете запустить бота одной командой. Это изолирует приложение и упростит управление зависимостями.

### Подготовка
1. Убедитесь, что у вас есть файл `.env` с корректными данными.
2. Файл `docker-compose.yml` и `Dockerfile` должны находиться в корне проекта.
...
### Преимущества Docker для Full-stack серверов
- **Изоляция**: Бот не конфликтует с версиями Node.js других приложений.
- **Персистентность**: База данных и логи хранятся в томах (`./data` и `./logs`), что позволяет легко делать бэкапы.
- **Автозапуск**: Docker автоматически поднимет контейнер после перезагрузки сервера.
